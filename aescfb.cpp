...
void AesCFB::encryptFile(const string& path, Keygen* keygen, const Encoding encoding)
{
...
void AesCFB::decryptFile(const string& path, Keygen* keygen, const Encoding encoding)
{
    string filename, output;
    const SecByteBlock& key = keygen->getKey();
    const SecByteBlock& iv = keygen->getIv();
