...
void AesGCM::encryptFile(vector<string>* paths, Keygen* keygen, const Encoding encoding) noexcept(false)
{
    try {
        const SecByteBlock& key = keygen->getKey();
        const SecByteBlock& iv = keygen->getIv();
...
void AesGCM::decryptFile(vector<string>* paths, Keygen* keygen, const Encoding encoding) noexcept(false)
{
    try {
        const SecByteBlock& key = keygen->getKey();
        const SecByteBlock& iv = keygen->getIv();
